...
export default function PriceCalculator({ prices, salesStats }: Props) {
  ...
  const result = baseValue != null ? (baseValue * percent) / 100 : null;
  ...
  return (
    <div className="card" style={{ padding: 22 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
          {baseOptions.map(opt => (
            <button
              key={opt.key}
              className={`pill${baseKey === opt.key ? ' active' : ''}`}
              onClick={() => setBaseKey(opt.key)}
              style={{ fontSize: 13 }}
            >
              {opt.label}
              {opt.value != null && (
        ...
        {baseKey === 'custom' && (
          <div style={{ marginTop: 10, position: 'relative', width: 140 }}>
            ...
            <input
              ...
              style={{
                ...
                padding: '9px 12px 9px 26px', fontSize: 14,
      ...
      <div style={{
        ...
        <div>
          <p style={{ fontSize: 13, color: 'var(--text2)', margin: '0 0 2px' }}>
            {percent}% of {fmt(baseValue)}
          </p>
